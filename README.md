...
| `Authorization:WritePermission` | No | `vitally:write` | Permission required for create/update operations (HTTP POST/PUT). |
...
- **Per-user audit trail** (`Audit:*`) — every action is logged at the choke point with the authenticated user (email + `sub`), HTTP verb, target resource path and outcome (denied attempts included). Because all users share one Vitally key, Vitally's own log can't attribute actions to individuals; this server-side record can, and is queryable in Application Insights / Log Analytics. Request bodies are never logged (no PII leakage).
