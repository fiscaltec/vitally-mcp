...
FISCAL employees point their MCP client at `https://vitally.fiscaltec.com/mcp`. The client handles the OAuth flow automatically on first use via the protected-resource metadata document. Everyone signs in with their FISCAL Entra account on both targets. See the current-state note at the top of this file.
...
- Binds `OAuthOptions` from `OAuth:` (provider-agnostic — works with Entra, Keycloak, Okta, etc.; only `OAuth:UpstreamResourceScope` differs between them, and it is a value rather than a branch).
...
**Validated against the live identity provider on 2026-08-22**, which the local run could not cover. MCP
Inspector 2.3.0 completed the whole flow against the server behind an HTTPS tunnel with
`PublicBaseUrl` set: metadata 200, DCR `201`, a real federated sign-in, `/oauth/callback` 302 carrying
...
|---|---|---|
| Provider it suits | a conforming RFC 8707 provider | Entra |
...
`Program.cs` sends **no `audience` parameter anywhere**, so something has to name the API. Under a
...
The relay half of that switch is retained deliberately, not vestigially: it is the RFC 8707 default,
and keeping Entra's deviation a value rather than a branch is what makes this server portable (#156).
...
the previous provider's path shapes and are now read from `{Authority}/.well-known/openid-configuration`
...
normalised on both sides and nothing else is — providers differ on it and Entra's carries none, so that
...
> the *Container App secret name* behind `SharedClientSecret` is now the **same** on both —
> `entra-oauth-client-secret` — since #156 normalised staging onto production's name and removed the
...
> | `SharedClientSecret` | `secretref:entra-oauth-client-secret` | *same* |
>
> `infra/terraform/variables.tf` defined these as `oauth_*` / `staging_oauth_*` pairs until #156
> collapsed them onto the shared `oauth_*` set, closing #102. Only `oauth_resource` and
> `public_base_url` remain per-target.

- `Authority` — the provider's OIDC **issuer** identifier (see the per-target table above; Entra's is `https://login.microsoftonline.com/75bd6050-92a8-4bde-a406-50000b310c86/v2.0`). It is *not* a prefix the endpoint URLs are built from: `{Authority}/.well-known/openid-configuration` is fetched and the endpoints come from that document. The trailing slash is whatever the provider's own issuer carries — Entra's has none.
...
> equal under the previous provider — its API identifier happened to carry a trailing slash, as does
> the form clients normalise to — so they read as one setting, and `infra/terraform/` fed both from a
> single `oauth_audience` variable. #108 split that variable, which is what makes the difference
> structural rather than a convention someone has to remember.
>
> | | Historical posture (retired provider) | Entra posture (both targets, live) |
> |---|---|---|
...
- `UpstreamResourceScope` — `https://vitally.fiscaltec.com/mcp.access` on **both** deployed targets. Setting it makes the proxy **terminate** the RFC 8707 `resource` parameter and name the API by this scope instead; leaving it empty relays `resource`, which is the RFC 8707 default and what a conforming provider expects. Entra is the outlier that requires it set. It stays a configured *value* rather than a branch in `Program.cs`, which is what lets this server front another provider without a code change (#156). One switch, because neither half works alone — see the `resource` section above. Validated at boot as a single whitespace-free token.
...
- `SharedClientSecret` — confidential-client secret for `SharedClientId`, injected server-side at `/oauth/token`. On both targets its value comes from the Key Vault secret `entra-mcp-client-secret`, but the app does **not** read the vault for it: the value was **copied** into a Container App secret named `entra-oauth-client-secret` on **both** targets (#156 normalised staging onto production's name) and `OAuth__SharedClientSecret` is a `secretRef` to that copy. That is not the `vitally-shared` pattern, and it means **rotating the Key Vault secret alone changes nothing the app sends** — the runbook carries the corrected, mechanically-verified procedure (#138). Two traps it records, both measured 2026-09-21: `az containerapp secret set` does **not** roll a revision, and **staging cannot rehearse the step that needs one** — staging is `minReplicas: 0` so a cold start reads the new value and the roll looks optional, while production's warm replica keeps the old value until restarted. One app registration serves both targets, so the final credential delete is global and cannot be rehearsed in isolation either: staging is the first half of the real rotation, not a dry run of it. It **expires 2027-03-01** — a hard outage date, but not for the reason the vault pattern would suggest: the app never reads the vault for it, so the failure is **Entra** rejecting the expired credential at `/oauth/token` with `invalid_client`, and no Key Vault state affects it. Rotation is in `docs/runbooks/entra-app-registration.md`.
...
- **A Graph failure degrades in one step, then denies** (#106, #108). The order is **fresh Graph → stale Graph → deny**: `GraphGroupPermissionResolver` keeps each successful lookup with the time it was resolved and, when a Graph call fails, serves that caller's last known-good set for up to `LiveGroupStaleSeconds` (default 1 h) before giving up and returning null — at which point `ToolAuthorizer` denies, explicitly and with a log line saying which of the two fail-closed routes was taken. Serving stale logs **one** warning carrying the caller's Entra **object id** — the same `oid` `CallerIdentity` resolves for the audit record, not the JWT `sub`, so a stale-serve line joins to the audit trail and to the membership that caused it — and how stale the result is in seconds. Never the email, never two lines per call.
  ...
  - **The claim tier used to sit beneath the stale cache and was removed at the cutover (#108).** While the provider's post-login hook minted the claim it genuinely authorised, which is why #106 added the stale cache *beneath* rather than in place of it. #108 removed the code that reads any claim while `LiveGroupCheck` is on, so no minted claim could authorise anyone; #156 then deleted the hook itself, so none is minted. A fall-through could therefore only ever have denied, leaving code that read like a working fallback and behaved like a silent denial. Do not reinstate it "as a safety net".
...
- `AuditLogger` is invoked from **`VitallyService.SendAsync`** (same choke point): `LogAction` after each upstream response and `LogDenied` on an RBAC denial. Records the caller's identity, HTTP verb, resource path (query string stripped) and status code via structured logging. **The identity is resolved in a specific order and only the first step yields an Entra object id** — `CallerIdentity.TryGetObjectId` (the `oid` claim alone, since #156), then the raw `sub`, then `ClaimTypes.NameIdentifier`, then the literal `unknown`; an unauthenticated caller is `anonymous`. The fallbacks guard against an unexpected token shape rather than a supported path — an Entra token always carries `oid` — and a consistent-but-opaque key beats none. Read a record keyed on anything but a GUID as *"this token shape had no object id"*, not as a defect — so the named properties are *shaped* to become queryable dimensions — and **on production they are now genuinely queryable**, in `AppEvents`, since the 2026-09-25 switch-on (#147). ⚠️ **Staging is still the old situation**: no `ApplicationInsights__ConnectionString`, so its records go to stdout and are retained nowhere. Records predating 2026-09-25 do not exist anywhere — nothing was ever persisted — so the trail begins at that date. **Never log upstream request/response *bodies* here — they can contain customer PII (traits, transcripts).** Note the boundary precisely, because the two rules meet at writes: what is excluded is the **HTTP payload** `VitallyService` exchanges with Vitally. **Tool arguments are in scope** under the policy below, including a create/update `jsonBody` — *"alice set these fields on this account"* is the audit record for a modification, and omitting it would lose the thing the trail exists to show. This is the attribution mechanism while a single shared Vitally key is in use.
  ...
  >
  > **The tool-call record is implemented (#147), and it is now the primary record.** `AuditLogger.LogToolCall` is emitted once per **executed** `tools/call` from a filter in `Program.cs` — ⚠️ *executed*, because the SDK's `[Authorize]` checkpoint rejects an out-of-tier call **outside** the filter pipeline, so a **tier-denied call produces only `LogToolCallDenied`**: caller, tool and required permission, but no arguments, correlation id or touched records. The trail is not blind on a denial, but it cannot say what the caller was reaching for. Deliberate boundary, not an oversight. It carries the caller's object id, the tool name, the arguments, the record ids touched, `recordsFetched`/`idsRecorded`/`truncated`, outcome, duration, a correlation id and the permission tier the authorisation decision actually used. `LogAction` is kept as **corroboration** — it shows what the server actually did — rather than as the mechanism.
  >
  > Four details worth knowing before changing any of it, each of which is a test:
  >
  > | | |
  > |---|---|
  > | Ids come from the **raw** upstream body | `GetResourcesAsync` applies `FilterJsonFields` afterwards, so `fields=name` yields results with no `id` at all. Capturing downstream would name nobody on exactly the calls a narrow projection was used for |
  > | `truncated` ≠ the id cap | It means the **pager** stopped at `MaxAutoPageFetches`, so the matching total is *unknowable* (Vitally exposes only `next`). The id cap shortens the list while `recordsFetched` stays true |
  > | `ToolCallAuditContext` is **scoped** | The tool's `VitallyService` writes into it and the filter reads it back. A singleton merges callers together; a transient records nothing. Pinned by `ToolCallAuditCompositionTests`, not unit tests |
  > | `PermissionTier` is written by `ToolAuthorizer` | The tier must be the one the decision was made against. Re-deriving it would mean a second Graph lookup that could answer differently — and entitlement is resolved live, so nothing could afterwards say which was right. **Unbackfillable**, which is why it could not wait |
  >
  > ⚠️ **`TierServedStale` is tri-state and is `unknown` today.** `GraphGroupPermissionResolver` serves a retained copy during a Graph outage and logs that it did, but does not report it back through `IGroupPermissionResolver`. Recording `false` would assert the tier was fresh when nothing checked. Threading the real signal out touches the fail-closed entitlement path and is tracked as #161.
  >
  > **Routing is built, and switched on by configuration.** Every audit record carries the `microsoft.custom_event.name` attribute, which the Azure Monitor OpenTelemetry exporter uses to write it to **`AppEvents`** rather than `AppTraces`; `Program.cs` registers the exporter and suppresses the whole `VitallyMcp.AuditLogger` category from the console **only when `ApplicationInsights__ConnectionString` is set** — **set on production 2026-09-25, still unset on staging**, whose records therefore go to stdout as before. ⚠️ **#142's console export is ungated on production as of that flip**, because the console stream is now customer-data-free: a breadcrumb stays there carrying the object id, tool name, outcome and correlation id and nothing else, since OpenTelemetry export is asynchronous and a lost export cannot be detected in-process. Design: `docs/superpowers/specs/2026-09-17-logging-observability-design.md`.
  >
  ...
  - **Every record is keyed on `oid`, and nothing parses a subject for one.** An Entra token always carries `oid`, so a record keyed on anything but a GUID means an unexpected token shape rather than a second provider's format. The federated-`sub` fallback that once existed for the retired provider was removed by #156; no record predating that is affected, because none was ever persisted (see the Logs row in the Deployment table).
...
Both targets hold that secret under the same Container App secret name, `entra-oauth-client-secret`.
They differed until #156, because production's was added alongside a retained rollback credential
rather than overwriting it; normalising them was part of retiring that rollback.
...
This server's objects at the previous provider — the client, both Resource Servers and the post-login
Action — were **deleted on 2026-09-22** as #156's final step.

⚠️ **Two things that look related and must not be removed.** The provider *tenant* itself stays: it
hosts Simple Asset System, its API and a Terraform client, none of them ours. So does the **Entra**
enterprise application that federates sign-in into that tenant — it is an Entra object rather than
one of this server's, and those other applications sign in through it.

Two consequences worth stating, because both were load-bearing constraints until #156 and are not any
more:

- **`CallerIdentity` reads the `oid` claim and nothing else.** The federated-`sub` fallback existed
  because a rolled-back token would have carried no `oid`; it was removed with the rollback. A token
  without `oid` now resolves to null and the caller fails closed.
- **The `permissions` claim cannot authorise anyone.** #108 removed the code that reads any claim
  while `LiveGroupCheck` is on, and nothing mints such a claim now. Entitlement comes from Graph,
  full stop.

The five `OAuth__*` values each target runs are in the configuration table above and in
`infra/terraform/variables.tf`. `OAuth__UpstreamResourceScope` remains a *value* rather than a
branch — the empty setting is the RFC 8707 relay default, kept so this server can front a conforming
provider without a code change, with Entra as the configured outlier.
...
#### If an identity change fails

Fix it forwards. There is no provider to revert to, so a failure is corrected in the Entra app
registration, the Gate 1 group assignments or the Container App configuration —
`docs/runbooks/entra-app-registration.md` is the reference for all three, and
`docs/runbooks/entra-cutover-staging-validation.md` is the acceptance suite.

⚠️ **The failure that hides from every automated check is a client-secret mismatch.** The app boots,
`/health` returns 200 and unauthenticated `/mcp` still returns 401, because none of those exercises
the credential. It surfaces only at `/oauth/token`, as `invalid_client`, for every user at once. Both
targets hold that secret as the Container App secret `entra-oauth-client-secret` — a **copy** of the
Key Vault secret `entra-mcp-client-secret`, so rotating the vault alone changes nothing the app sends
(#138). Verify a secret change by signing in, never by a health probe.
