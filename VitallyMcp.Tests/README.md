...
| `LoggingFilterTests` | Pins the log-level filters set in `Program.cs`. ⚠️ #143 framed the `System.Net.Http.HttpClient.*` filter as a PII control; **that premise was wrong** — .NET redacts query *values* (`?*`), so search terms never reached the logs. The filters are noise reduction, 86.8% of console bytes between them. Composes the real host and asserts on the concrete `.LogicalHandler` / `.ClientHandler` categories that appear in the live stream, taking the discovery client's name from `UpstreamOidcMetadata.HttpClientName` rather than a literal. Every claimed name is then **proven against the composed host** (`EveryClaimedHttpClientCategory_IsOneTheHostActuallyCreates`) by bringing each client up the way `Program.cs` does and recording the categories the framework asks for — the typed ones by resolving the service, never by asking the factory for a derived name. Without that, a wrong name passes every other assertion here, because the prefix filter reports *invented* `System.Net.Http.HttpClient.*` categories as disabled too — which is exactly how the discovery client's name was wrong and green earlier in #148. Each category is asserted quiet at `Information` **and** still enabled at `Warning`, because a regression to `None` would pass the first check while hiding genuine faults. One test pins that `AuditLogger` still logs at `Information` — a filter on the wrong prefix would delete the audit trail while looking like tidying. |
