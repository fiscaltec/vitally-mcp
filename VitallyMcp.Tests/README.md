...
| `LoggingFilterTests` | Pins the log-level filters set in `Program.cs`. The `System.Net.Http.HttpClient.*` filter is a **PII control, not noise reduction**: at `Information` those categories log the outbound request URI *including its query string*, and `Search_users` / `Search_admins` put caller-supplied search terms there (#143). Composes the real host and asserts on the concrete `.LogicalHandler` / `.ClientHandler` categories that appear in the live stream, taking the discovery client's name from `UpstreamOidcMetadata.HttpClientName` rather than a literal. Each category is asserted quiet at `Information` **and** still enabled at `Warning`, because a regression to `None` would pass the first check while hiding genuine faults. One test pins that `AuditLogger` still logs at `Information` — a filter on the wrong prefix would delete the audit trail while looking like tidying. |
