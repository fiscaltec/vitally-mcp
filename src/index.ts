...
if (!VITALLY_SUBDOMAIN || !VITALLY_API_KEY) {
    console.error("Please set VITALLY_SUBDOMAIN and VITALLY_API_KEY environment variables");
    process.exit(1);
}
...
async function vitallyRequest(endpoint: string, options: RequestInit = {}) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Vitally API error: ${response.status} ${response.statusText} - ${errorText}`);
    }
...
server.tool(
    "list-accounts",
    {
        limit: z.number().min(1).max(100).optional().default(50).describe("Number of items to return (max 100)"),
        from: z.string().optional().describe("Cursor from previous request for pagination"),
        sortBy: z.enum(["updatedAt", "createdAt"]).optional().default("updatedAt").describe("How to order the results"),
    },
...
server.tool(
    ...
    async ({ accountId, useExternalId }) => {
        const endpoint = useExternalId ? `/accounts/${accountId}` : `/accounts/${accountId}`;
...
server.tool(
    ...
    async ({ name, externalId, limit }) => {
        ...
        if (name) {
            ...
            const filtered = data.results?.filter((account: any) =>
            ...
            return {
                content: [{
                    type: "text",
                    text: JSON.stringify({ results: filtered, total: filtered.length }, null, 2)
                }]
            };
        }
...
server.tool(
    "list-tasks",
    {
        ...
        organizationId: z.string().optional().describe("Filter tasks by organization ID"),
        limit: z.number().min(1).max(100).optional().default(50).describe("Number of items to return (max 100)"),
        from: z.string().optional().describe("Cursor from previous request for pagination"),
...
server.tool(
    "list-notes",
    {
        ...
        organizationId: z.string().optional().describe("Filter notes by organization ID"),
        limit: z.number().min(1).max(100).optional().default(50).describe("Number of items to return (max 100)"),
        from: z.string().optional().describe("Cursor from previous request for pagination"),
        sortBy: z.enum(["updatedAt", "createdAt"]).optional().default("updatedAt").describe("How to order the results"),
    },
...
server.tool(
    "list-conversations",
    {
        accountId: z.string().optional().describe("Filter conversations by account ID"),
        organizationId: z.string().optional().describe("Filter conversations by organization ID"),
        limit: z.number().min(1).max(100).optional().default(50).describe("Number of items to return (max 100)"),
        from: z.string().optional().describe("Cursor from previous request for pagination"),
        sortBy: z.enum(["updatedAt", "createdAt"]).optional().default("updatedAt").describe("How to order the results"),
    },
...
server.tool(
    ...
    async ({ organizationId, useExternalId }) => {
...
server.tool(
    "list-projects",
    {
        ...
        organizationId: z.string().optional().describe("Filter projects by organization ID"),
        limit: z.number().min(1).max(100).optional().default(50).describe("Number of items to return (max 100)"),
        from: z.string().optional().describe("Cursor from previous request for pagination"),
        sortBy: z.enum(["updatedAt", "createdAt"]).optional().default("updatedAt").describe("How to order the results"),
    },
...
server.tool(
    "list-nps-responses",
    {
        accountId: z.string().optional().describe("Filter NPS responses by account ID"),
        organizationId: z.string().optional().describe("Filter NPS responses by organization ID"),
...
server.tool(
    ...
    {
        endpoint: z.enum([
            "accounts", "users", "tasks", "notes",
            "conversations", "organizations", "projects", "npsResponses"
        ]).describe("The endpoint to paginate through"),
        accountId: z.string().optional().describe("Filter by account ID (for supported endpoints)"),
        organizationId: z.string().optional().describe("Filter by organization ID (for supported endpoints)"),
        userId: z.string().optional().describe("Filter by user ID (for NPS responses)"),
        maxPages: z.number().min(1).max(10).optional().default(5).describe("Maximum number of pages to fetch (safety limit)"),
        pageSize: z.number().min(1).max(100).optional().default(100).describe("Items per page"),
        sortBy: z.enum(["updatedAt", "createdAt"]).optional().default("updatedAt").describe("How to order the results"),
    },
    async ({ endpoint, accountId, organizationId, userId, maxPages, pageSize, sortBy }) => {
        ...
        try {
            while (pageCount < maxPages!) {
                ...
                if (accountId && ["tasks", "notes", "conversations", "projects", "npsResponses"].includes(endpoint)) {
                    apiEndpoint = `/accounts/${accountId}/${endpoint}`;
                } else if (organizationId && ["accounts", "tasks", "notes", "conversations", "projects", "users", "npsResponses"].includes(endpoint)) {
...
server.resource(
    "account-summary",
    new ResourceTemplate("vitally://accounts/{accountId}/summary", { list: undefined }),
    async (uri, { accountId }) => {
        try {
            // Get account details
            ...
            // Get recent tasks
            const tasks = await vitallyRequest(`/accounts/${accountId}/tasks?limit=10`);

            // Get recent notes
            const notes = await vitallyRequest(`/accounts/${accountId}/notes?limit=5`);

            // Get recent conversations
            const conversations = await vitallyRequest(`/accounts/${accountId}/conversations?limit=5`);
...
server.resource(
    ...
    new ResourceTemplate("vitally://organizations/{organizationId}/summary", { list: undefined }),
    async (uri, { organizationId }) => {
        try {
            // Get organization details
            const organization = await vitallyRequest(`/organizations/${organizationId}`);

            // Get accounts for this organization
            const accounts = await vitallyRequest(`/organizations/${organizationId}/accounts?limit=20`);

            // Get recent tasks
            const tasks = await vitallyRequest(`/organizations/${organizationId}/tasks?limit=10`);

            // Get recent notes
            const notes = await vitallyRequest(`/organizations/${organizationId}/notes?limit=5`);
